...
/// Resolve the guard parameter for a command handler.
///
/// Commands listed in `public = [...]` never receive a guard; other
/// commands use the `commands` per-operation override or the
/// container-level `guard`.
fn command_guard_param(entity: &EntityDef, cmd: &CommandDef) -> TokenStream {
    let api_config = entity.api_config();
    let is_public = api_config.public_commands.contains(&cmd.name);
    if is_public {
        return TokenStream::new();
    }
    match api_config.guard_for("commands") {
        Some(guard) => quote! { _guard: #guard, },
...
fn generate_handler_without_id(
    ...
    let vis = &entity.vis;
    let guard_param = command_guard_param(entity, cmd);
    let doc = format!(
